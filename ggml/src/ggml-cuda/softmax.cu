...
static __global__ void soft_max_f32(
    ...
    __shared__ float buf_iw[WARP_SIZE]; // static shared memory for inter-warp reduction (separate from dynamic)
    extern __shared__ float data_soft_max_f32[]; // dynamic shared memory for vals cache only
    // shared memory buffer to cache values between iterations:
    float * vals = use_shared ? data_soft_max_f32 : dst;
...
#endif // !defined(GGML_USE_ILUVATAR)
        {
            const size_t nbytes_shared_low = 0; // buf_iw now in static shared memory
